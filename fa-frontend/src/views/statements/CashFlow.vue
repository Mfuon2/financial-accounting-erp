...
async function load() {
  ...
  try {
    const params = isDemo.value
      ? {}
      : { entityId: entityId.value, periodId: selectedPeriodId.value, startDate: selectedPeriod.value?.startDate, endDate: selectedPeriod.value?.endDate }
    if (!isDemo.value && (!params.entityId || !params.startDate)) { loading.value = false; return }
    const res = await reports.cashFlow(params)
...
const sections = computed(() => {
  ...
  return [
    ...
    { type: 'subtotal', label: 'Net cash from operating',     current: Number(d.operatingActivities) },
    { type: 'section',  label: 'Investing activities' },
    { type: 'subtotal', label: 'Net cash from investing',     current: Number(d.investingActivities) },
    { type: 'section',  label: 'Financing activities' },
    { type: 'subtotal', label: 'Net cash from financing',     current: Number(d.financingActivities) },
    { type: 'total',    label: 'Net change in cash',          current: Number(d.netChangeInCash) },
    { type: 'line',     label: 'Cash at beginning of period', indent: 0, current: Number(d.openingCash) },
    { type: 'total',    label: 'Cash at end of period',       current: Number(d.closingCash) },
  ]
})
...
async function exportPdf() {
  ...
  try {
    const params = isDemo.value ? {} : { entityId: entityId.value, periodId: selectedPeriodId.value, startDate: selectedPeriod.value.startDate, endDate: selectedPeriod.value.endDate }
    await reports.pdfCashFlow(params, 'cash-flow.pdf')
...
<template>
  <div class="page">
    <PageHeader title="Statement of Cash Flows (IAS 7 — Indirect)" :meta="periodLabel">
      <template #default>
        <SearchableSelect
